...
| 0.2 | Общий layout | 00, 07 | Шапка/навигация: логотип «Log Life», ссылка на список дел, иконка «Профиль» |
| 0.3 | Стили и темы | 00 | CSS-переменные, mobile first, читаемость на десктопе |
...
| 3.2 | Поля дела | Эмодзи (выбор/ввод), название, описание (многострочное, необязательное) |
...
| 4.6 | Блок «Заметки» | Поле внизу формы, свободный текст |
| 4.7 | Кнопка «Добавить запись» | Валидация обязательных → `api.deeds.createRecord(deedId, { record_date, record_time, notes, answers })` |
| 4.8 | После сохранения | Редирект на страницу дела или список дел, при необходимости — сообщение об успехе |
...
| 5.4 | Просмотр записи | Все ответы по блокам, дата, время, заметки. Устаревшие ответы (блок удалён) — показывать со снимком ([13](requirements/13-rules-editing-deed-data.md)) |
| 5.5 | Редактирование записи | Редактирование даты, времени, заметок; при необходимости — ответов. `api.records.update(id, body)` |
...
- **Shadcn UI**: компоненты в `src/components/ui/`, стили — Tailwind и CSS-переменные в `src/index.css`.
