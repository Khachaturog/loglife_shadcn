...
Краткое описание бизнес-логики приложения по документам из этой папки (см. [README](README.md)).
...
- **Типы блоков:**
  - Число — одно число.
  - Текст (абзац) — текстовый ответ (многострочное поле ввода).
...
### 3.1 Упорядочивание ([05](05-rules-ordering.md))
...
### 3.2 Обязательность ([06](06-rules-required.md))
...
### 3.3 Поведение при редактировании дела ([13](13-rules-editing-deed-data.md))
...
- **Смена типа блока:** при редактировании дела при смене типа существующего блока открывается модалка превью «Было → Стало» по записям; после подтверждения тип и (при поддерживаемой паре) ответы обновляются в базе. Если автоперенос не применялся, при просмотре запись может попадать в «устаревшее» до ручной актуализации (см. [13](13-rules-editing-deed-data.md)).
...
**Навигация:** на мобильных устройствах список дел и профиль доступны через нижнюю панель вкладок (Дела / Профиль); см. [15-layout-navigation](15-layout-navigation.md).

### 4.1 Список дел ([07](07-screen-list-deeds.md))
...
### 4.2 Создание и редактирование дела ([08](08-screen-create-edit-deed.md))
...
### 4.3 Заполнение формы ([09](09-screen-fill-form.md))

- Контекст дела — карточка с эмодзи, названием и описанием. Редактируемые дата и время записи (по умолчанию — текущие).
- Блоки в порядке из дела; каждый рендерится по типу (число, текст (абзац), один/несколько из списка, шкала, да/нет). Обязательные помечаются.
- **Быстрый выбор из недавних записей:** при открытии экрана подгружаются последние 10 записей этого дела. Для блоков «Число» и «Один из списка» под полем показываются чипы с уникальными значениями из этих записей (не более нескольких штук; набор по недавности использования в записях). На экране чипы с числами упорядочены **от большего к меньшему**, чипы вариантов списка — **по подписи по убыванию**. Пока поле пустое, пользователь может нажать чип и подставить значение; после выбора или ввода чипы для этого блока скрываются; когда ответ снова пустой (в т.ч. после «Сбросить», ручного стирания числа или шага «−» до пустого числа), подсказки снова показываются. Для списка предлагаются только варианты, которые есть в текущей конфигурации блока (удалённые варианты из истории не показываются).
...
### 4.4 Просмотр дела ([14](14-screen-deed-view.md))

- Эмодзи, название, описание. Блок **история записей** по этому делу (дата, время, превью ответов).
- Краткая **аналитика** по делу: текущий и максимальный стрик (по календарным дням с записями), число **записей**, пришедших на будни (пн–пт) и на выходные (сб–вс) — день недели по дате записи считается так же, как при отображении даты в истории; несколько записей в один день каждая учитывается в своей категории.
- Действия: **добавить запись**, **редактировать дело**, **удалить дело** (с подтверждением).

### 4.5 История записей ([10](10-screen-history.md))

- Реализована на странице просмотра дела: список записей по делу (дата, время, превью).
- Открытие записи → просмотр полных ответов и служебных полей; при необходимости — редактирование даты, времени, заметок (и при необходимости ответов, с учётом правил устаревания из [13](13-rules-editing-deed-data.md)).
- **Удалить запись** — с подтверждением (диалог в интерфейсе, не нативный `window.confirm`, чтобы работало и во встроенных браузерах IDE); после подтверждения запись удаляется, навигация «Назад» ведёт к делу или к экрану истории в зависимости от сценария входа.

### 4.6 Профиль ([12](12-screen-profile.md))
...
Документ составлен по: [00-overview](00-overview.md) – [14-screen-deed-view](14-screen-deed-view.md).
